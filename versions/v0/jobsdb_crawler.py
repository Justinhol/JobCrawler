...
class JobsDBCrawler:
    ...
    def __init__(self):
        ...
        self.headers = {
            ...
            "Referer": "https://hk.jobsdb.com/jobs?classification=6123%2C6281%2C1200%2C6251%2C6304%2C1203%2C1204%2C1225%2C6246%2C6261%2C1223%2C6362%2C6043%2C1220%2C6058%2C6008%2C6092%2C1216%2C1214%2C6317%2C1212%2C1211%2C1210%2C6205%2C1209%2C6263%2C6076%2C1206%2C6163%2C7019",
    ...
    def crawl_all_pages(self, total_pages):
        ...
        folder_name = current_time + "_" + str(len(all_ids))
        ...
        # 将所有ID平均分配给6个线程
        chunk_size = len(all_ids) // 6
        job_chunks = [all_ids[i:i + chunk_size]
                      for i in range(0, len(all_ids), chunk_size)]

        # 如果有余数，将剩余的ID添加到最后一个chunk
        if len(job_chunks) > 6:
            job_chunks[5].extend(job_chunks[6:])
            job_chunks = job_chunks[:6]
        ...
        # 合并CSV文件
        print("\n合并CSV文件...")
        combined_csv_path = f'./{folder_name}/{folder_name}.csv'
        df_list = []

        for i in range(6):
            thread_csv = f'./{folder_name}/thread_{i}.csv'
            if os.path.exists(thread_csv):
                df = pd.read_csv(thread_csv, encoding='utf-8')
                df_list.append(df)
                os.remove(thread_csv)  # 删除临时CSV文件
        ...
        if all_failed_ids:
            ...
            with open(f'./{folder_name}/failed_ids.json', 'w', encoding='utf-8') as f:
                json.dump(all_failed_ids, f, ensure_ascii=False, indent=2)
            print(f"失败的ID已保存到 {folder_name}/failed_ids.json")
