...
class JobsDBCrawler:
    ...
    def crawl_all_pages(self, total_pages):
        ...
        # 记录失败的ID
        if all_failed_ids:
            with open(f'./{folder_name}/failed_ids.json', 'w') as f:
                json.dump(all_failed_ids, f)

        # 重命名文件夹（使用实际成功数量）
        # 路径包含data目录
        new_folder_name = f"data/{current_time}_{total_success}"
    ...
    def process_csv_extraction(self, folder_name=None):
        ...
        # 3. 合并CSV文件
        print("\n合并CSV文件...")
        self.merge_csv_files(folder_name)
    ...
    def get_latest_data_folder(self):
        ...
        folders = [f for f in os.listdir(data_dir) if os.path.isdir(
            os.path.join(data_dir, f)) and f[0].isdigit()]
        if not folders:
            return None
    ...
    def process_jobs_with_configpool(self, job_ids, original_data_folder, max_retries=1, is_retry=False):
        ...
        for attempt in range(max_retries):
            ...
            for idx, job_id in enumerate(job_ids):
                ...
                # 处理单个职位
                file_path = f'{original_data_folder}/{job_id}.json'
                ...
                try:
                    ...
                    if os.path.exists(file_path):
                        os.remove(file_path)
                    all_failed_ids.append(job_id)
                    print(f"{len(all_failed_ids)}: 处理职位 {job_id} 失败: {str(e)}")

                # 更新进度
                total_processed += 1
                if total_processed % self.config.job_progress_step == 0:
                    print(f"总进度: {total_processed}/{len(job_ids)}")
